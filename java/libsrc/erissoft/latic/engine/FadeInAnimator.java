...
public class FadeInAnimator extends LightAnimator {

    public FadeInAnimator(int step, Color color) { 
        step_ = step;
        color_ = color;
    ...
    public boolean isDone() { return  brightness_ >= Light.MAX_BRIGHTNESS; }

    protected Light getLight_() {
        brightness_ += step_;

        if (brightness_ > Light.MAX_BRIGHTNESS)
    ...
    public AnimatorInterface make() {
        return new FadeInAnimator(step_, color_);
    }
